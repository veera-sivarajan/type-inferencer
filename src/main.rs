...
mod tests {
    ...
    fn test(output: &[Substitution], expected: &[Substitution]) -> bool {
        !expected.iter().any(|e| !output.contains(e)) && output.len() - 1 <= expected.len()
    }
    ...
    #[test]
    fn test_binary() {
        let exp = Expr::make_binary(&Expr::Number(1), Operation::Add, &Expr::Number(2));
        let subs = vec![
    ...
    fn test_function() {
        ...
        let subs = vec![
            ...
            Substitution::new(&Term::Expr(f.clone()), &Term::make_arrow(&Term::Num, &Term::Num)),
        ];
